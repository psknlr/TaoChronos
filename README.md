...
TaoChronos 把 944 部中医古籍（约 1.21 亿字、203 万段）整理成一个**可检索、可断代、可溯源**的语料库，并在其上提供两层能力：
...
| 數位中醫校書郎 CMETA（精校本） | CC BY 4.0（校订成果）；底本公有领域 | **10 部**，137 万字 | 逐页对影像精校：伤寒论（赵开美本台北故宫本与安政覆刻本、康平本、康治本）、金匮（吴迁本、邓珍本）、素问（顾从德本）、灵枢（赵府居敬堂本）、古今医案按、名医类案；段落带页码，向访客开放影像者附页面影像地址 |
...
上面两表的实例数值取自 926 部的语料（2.0 版）；本版新增的 18 部录本（CMETA、东亚医学协会、医方类聚）会使部分计数略有变化，
版本谱系一行已按 944 部重算。
