...
TaoChronos 把 946 部中医古籍（约 1.21 亿字、203 万段）整理成一个**可检索、可断代、可溯源**的语料库，并在其上提供两层能力：
...
| 笈成《笈成檢閱系統》v1.4.8 | 使用者提供，仅本地研究 | **798 部**（857 部剔除 59 部），约 9235 万字 | 整理者新式标点；标题、方剂块、注疏、校记、缺字按标记解析 |
...
| 维基文库 Category:中醫 与 Category:醫書（经 Wikimedia 转储） | CC BY-SA 4.0 | **16 部**（627 部中去重后新增） | 天回医简、五十二病方、民国三种“古本”伤寒杂病论、东医宝鉴、医方类聚（朝鲜，1445）等 |
...
| 影像见证：日本国文学研究资料馆（研医会、庆应与京大富士川文库、东大含鹗轩文库、九大、东北大）、柏林国家图书馆文树德藏品、美国国会图书馆中文善本、早稻田古典籍（日本国会图书馆：检索器已备，书目待建） | 各馆授权逐条记录 | 书目记录 书目 **9,638 条**（关联到 168 部著作的 446 条） | 只收题名、年代、刻/写、藏所、索书号、IIIF 清单与授权，按题名关联到库中著作；不下载影像 |

合计 **946 部、约 1.21 亿字、203 万段**。

- **准入**：语料只收历史文献。当代出版物、当代名医著作、现代校注本（含亡佚古书的现代辑复本）逐部人工复核后剔除
  （`corpus/catalog/exclusions.yaml`，自动筛查补其余）；民国著作（1912—1949）作为历史文献保留。近世才问世而托名古代的传本
